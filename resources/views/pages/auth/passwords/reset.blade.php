...
@push('css')
    <style>
        ...
        #auth_reset_password_form .card-body {
            padding: 0;
        }
    </style>
@endpush
        ...
        <div class="d-flex flex-column-fluid flex-lg-row-auto justify-content-center justify-content-lg-end p-12">
            <div class="bg-body d-flex flex-center rounded-4 w-md-600px p-10">
                <div class="w-md-400px">
                    ...
                    @endif
                    {{
                        \Bws\Core\Facades\Form::id('auth_reset_password_form')
                            ->className('form w-100')
                            ->routeAction('admin.password.update')
                            ->attributes(['novalidate'=>'novalidate'])
                            ->method('POST')
                            ->fields([
                                \Bws\Core\Facades\Fields\HtmlField::content(function () {
                                    echo    '<div class="text-center mb-11">
                                                    <h1 class="text-dark fw-bolder mb-3">'.trans('bws/core::core.auth.reset_password').'</h1>
                                ...
                                \Bws\Core\Facades\Fields\FieldsGroup::className('fv-row mb-8')
                                    ->fields([
                                        \Bws\Core\Facades\Fields\TextField::make('token', 'hidden')
                                            ->setValue($token)
                                    ]),
                                \Bws\Core\Facades\Fields\FieldsGroup::className('fv-row mb-8')
                                    ->fields([
                                ...
                                \Bws\Core\Facades\Fields\FieldsGroup::className('d-grid mb-10')
                                    ->fields([
                                        \Bws\Core\Facades\Fields\ActionField::make('auth_reset_password_form_submit', 'submit')
                                            ->className('btn btn-primary')
                                            ->label(trans('bws/core::core.auth.reset_password'))
                                    ]),
                            ])
                            ->render()
                    }}
