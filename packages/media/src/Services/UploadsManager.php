...
class UploadsManager
{
    ...
    {
        return $this->mimeType->getMimeType(File::extension(bws_media()->getRealPath($path)));
    }
    ...
    {
        if (!bws_media()->isChunkUploadEnabled() || !$file) {
            return Storage::put($this->cleanFolder($path), $content);
        }

        $currentChunksPath = bws_media()->getConfig('chunk.storage.chunks') . '/' . $file->getFilename();
        $disk = Storage::disk(bws_media()->getConfig('chunk.storage.disk'));
