...
use Bws\Assets\Filters\FilterInterface;
use Bws\Assets\Loaders\LoaderInterface;
use Bws\Assets\Notifiers\NotifierInterface;
...
class Assets
{
     ...
     * @return Assets
     */
    public function setPublicAssets($public_assets)
    {
        $this->add($public_assets);
        return $this;
    }
     ...
     * @return bool
     */
    private function needsPurge(array | FileAttributes $file, $timestamp)
    {
        $path = is_array($file) ? $file['path'] : $file->path();
        $times = is_array($file) ? $file['timestamp'] : $file->lastModified();
        $eligible = preg_match(self::REGEX_JS, $path) || preg_match(self::REGEX_CSS, $path);

        return $eligible && $times <= $timestamp;
    }
}
